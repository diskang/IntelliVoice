...
void PaintVideo::setData (const void *image_in, ssi_size_t size_, ssi_time_t time)
{
	...
	if(_flipForOutput)
	{
		...
		BYTE *dstptr = _image_out_tmp + (_height - 1) * stride;
...
void PaintVideo::mouseDown(ICanvas *canvas, ssi_point_t position, IWindowClient::KEY vkey) {
}

void PaintVideo::keyDown(ICanvas *canvas, IWindowClient::KEY key, IWindowClient::KEY vkey) {
}
...
void PaintVideo::setBackground(ssi_rgb_t color) {
	...
	::DeleteObject(_back_brush);
	_back_brush = ::CreateSolidBrush(color);
}

void PaintVideo::paint(ICanvas *canvas, ssi_handle_t hdc, ssi_rect_t rect)
{
	RECT r;
	...
	if (_draw_video) {
		paintAsVideoImage(ssi_cast(HDC, hdc), r);
	} else {
		::FillRect((HDC)hdc, &r, (HBRUSH)_back_brush);
	}
}
