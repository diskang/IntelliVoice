...

	{
		...
		for (ssi_size_t i = 0; i < sample_number; i++) {
			for (ssi_size_t j = 0; j < sample_dimension; j++) {
				value = *srcptr++;
				*dstptr++ = value;
				if (type == PaintSignalType::IMAGE && !fixed_min_max) {
...
void PaintSignal::keyDown(ICanvas *canvas, IWindowClient::KEY key, IWindowClient::KEY vkey) {

	resetLimits();
}

void PaintSignal::paint(ICanvas *canvas, ssi_handle_t context, ssi_rect_t area) {
...
void PaintSignal::paintAsPath(ssi_rect_t area) {
	...
	ssi_real_t x = *(data_scaledptr + x_ind);
	ssi_real_t y = *(data_scaledptr + y_ind);
...
#if HAS_CAIRO
void PaintSignal::paintAsImage2Buffer( cairo_surface_t** surf)
{
	...
	*surf= cairo_image_surface_create (
	...
	unsigned char* buffer = cairo_image_surface_get_data (*surf);
	
	int offset=3;//rgb
	for(int i=0; i < vals_x*vals_y; i++)
	{
		//*_colormap->getPen(*(data_scaled_ptr + y_inds[y]))
		
		uint32_t color=_colormap->getColor(*(data_buffer_scaled+i));
		
		char r_value=(color&0x000000FF)*1.0/256.0;
		char g_value=((color&0x0000FF00)>>8)*1.0/256.0;
		char b_value=((color&0x00FF0000)>>16)*1.0/256.0;
		
		buffer[i*offset]=r_value;
		buffer[i*offset+1]=g_value;
		buffer[i*offset+2]=b_value;
	}
...
void PaintSignal::paintAsImage(ssi_rect_t area) {
	...
	cairo_surface_t* img;
	paintAsImage2Buffer(&img);

	//cairo_scale  (cr, area.width, area.height);
	//cairo_translate (cr, -area.left, -area.top);
    //
