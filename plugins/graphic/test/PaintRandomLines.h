...
public:
	...
	void paint(ICanvas *canvas, ssi_handle_t hdc, ssi_rect_t rect) {
		...
		ssi_point_t *points = new ssi_point_t[n_tools];

		for (ssi_size_t i = 0; i < n_tools - 1; i++) {
			points[i].x = ssi_random(rect.width + 1u);
			points[i].y = ssi_random(rect.height + 1u);
		}
		...
		for (ssi_size_t i = 0; i < n_tools - 1; i++) {
			painter.line(*tools[i], points[i], points[i + 1]);
		}
		painter.end();
