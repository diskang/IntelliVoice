 ...
 */
char* ExpParser::parse(const char new_expr[], double &result)
{
    try
    {
    ...
    catch (Error err)
    {
        if (err.get_row() == -1)
        {
            _snprintf(ans_str, sizeof(ans_str), "Error: %s (col %i)", err.get_msg(), err.get_col());
        }
        else
        {
            _snprintf(ans_str, sizeof(ans_str), "Error: %s (ln %i, col %i)", err.get_msg(), err.get_row(), err.get_col());
        }
		return ans_str;
    }
 ...
 */
int ExpParser::get_operator_id(const char op_name[])
{
    // level 2
 ...
 */
double ExpParser::eval_operator(const int op_id, const double &lhs, const double &rhs)
{
    switch (op_id)
    {
        // level 2
 ...
 * evaluate a function
 */
double ExpParser::eval_function(const char fn_name[], const double &value)
{
    try
    {
 ...
 * evaluate a variable
 */
double ExpParser::eval_variable(const char var_name[])
{
