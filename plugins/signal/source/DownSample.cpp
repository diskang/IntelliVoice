...
void DownSample::transform (ITransformer::info info,
	...
	// down sample
	ssi_size_t iterations = sample_number / _options.factor;
	for (ssi_size_t i = 0; i < iterations; i++) {
		ssi_mean (_options.factor, sample_dimension, srcptr, dstptr);
		srcptr += sample_dimension * _options.factor;
		dstptr += sample_dimension;
	}
}
