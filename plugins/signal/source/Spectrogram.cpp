...
void Spectrogram::readFilterbank (const ssi_char_t *file, ssi_time_t sr) {
	...
	Matrix<ssi_real_t> *intervals = new Matrix<ssi_real_t> (n_banks, 2);
...
void Spectrogram::transform_enter (ssi_stream_t &stream_in,
	...
	if (!_filterbank) {
		if (_options.file[0] != '\0') {
			readFilterbank (_options.file, stream_in.sr);
		} else {
