...
Converts pulse signal to frequency.
...
public:
	...
	public:
		...
		Options ()
			...
			addOption ("delta", &delta, 1, SSI_REAL, "minimum distance between two extrema");		
			addOption ("minr", &min_rate, 1, SSI_REAL, "minimum rate");	
			addOption ("maxr", &max_rate, 1, SSI_REAL, "maximum rate");			
		};
...
public:
	...
	static const ssi_char_t *GetCreateName () { return "ssi_feature_Pulse"; };
	static IObject *Create (const ssi_char_t *file) { return new Pulse (file); };
	~Pulse ();

	Options *getOptions () { return &_options; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "pulse feature"; };
