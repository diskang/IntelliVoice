...
public:
	...
	public:
		...
		Options () : single (false), join (JOIN::OFF), cast(SSI_FLOAT) {
			...
			addOption ("single", &single, 1, SSI_BOOL, "apply a single expression to all dimensions otherwise expression is applied to each dimension (use d0, d1, ... to address specific dimension or d to address current dimension)");		
			addOption ("expression", expression, SSI_MAX_CHAR, SSI_CHAR, "expression string");					
			...
			addOption ("cast", &cast, 1, SSI_SIZE, "cast result at the end (CHAR = 1, UCHAR = 2, SHORT = 3, USHORT = 4, INT = 5, UINT = 6, LONG = 7, ULONG = 8, FLOAT = 9, DOUBLE = 10, LDOUBLE = 11, STRUCT = 12, IMAGE = 13, BOOL = 14");
		};
...
public:
	...
	static const ssi_char_t *GetCreateName () { return "ssi_filter_Expression"; };
	static IObject *Create (const ssi_char_t *file) { return new Expression (file); };
	~Expression ();

	Options *getOptions () { return &_options; };
	const ssi_char_t *getName () { return GetCreateName (); };
	const ssi_char_t *getInfo () { return "Applies mathematical expressions to a stream (see http://www.speqmath.com/tutorials/expression_parser_cpp/index.html)."; };
