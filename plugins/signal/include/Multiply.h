...
public:
	...
	public:
		...
		Options () : single (true), join (JOIN::OFF), factor (1.0f) {
			...
			addOption ("factor", &factor, 1, SSI_REAL, "multiplication factor");		
			addOption ("thresid", factorid, SSI_MAX_CHAR, SSI_CHAR, "name of tuple event to steer multiplication factor");
			addOption ("factors", factors, SSI_MAX_CHAR, SSI_CHAR, "multiplication factors separated by comma");					
			addOption ("join", &join, 1, SSI_INT, "join dimensions (0=off, 1=multiply, 2=sum up, 3=sum up squares");
		};
		...
		void setFactors (ssi_size_t n_inds, ssi_real_t *values) {
			...
			if (n_inds > 0) {
				...
				ssi_sprint (s, "%u", values[0]);
				strcat (factors, s);
				for (ssi_size_t i = 1; i < n_inds; i++) {
					ssi_sprint (s, ",%u", values[i]);
					strcat (factors, s);
				}
			}
		}

		void setFactorId (const ssi_char_t *factorid) {
			if (factorid) {
				ssi_strcpy (this->factorid, factorid);
			}
		}

		bool single;
		ssi_real_t factor;
		ssi_char_t factorid[SSI_MAX_CHAR];
...
public:
	...
	static const ssi_char_t *GetCreateName () { return "ssi_filter_Multiply"; };
