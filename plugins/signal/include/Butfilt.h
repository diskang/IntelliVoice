...
public:
	...
	public:
		...
		Options ()
			: type (BAND), order (1), norm (true), low (0), high (1), zero (false) {
			...
			addOption ("low", &low, 1, SSI_DOUBLE, "low cutoff frequency given either as normalized value in interval [0..1] or as an absolute value in Hz (see -norm)");
			addOption ("high", &high, 1, SSI_DOUBLE, "high cutoff frequency given either as normalized value in interval [0..1] or as an absolute value in Hz (see -norm)");
...
public:
	...
	static const ssi_char_t *GetCreateName () { return "ssi_filter_Butfilt"; };
	...
	virtual const Matrix<ssi_real_t> *getCoefs (ssi_time_t sr);
...
protected:
	...
	Matrix<ssi_real_t> *init_coefs (TYPE type, ssi_size_t order, ssi_time_t low, ssi_time_t high);
