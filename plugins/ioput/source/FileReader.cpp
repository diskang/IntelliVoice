...
void FileReader::setProvider (IProvider *provider) {
	...
	if (!read_file (_options.path)) {
		ssi_err ("an error occured while reading from file");
	}
...
bool FileReader::read_file (const ssi_char_t *path) {
	
	ssi_msg (SSI_LOG_LEVEL_DETAIL, "read file '%s'", path);

	if (!_file_stream_in.open (_stream, path, _n_meta, (void **) &_meta)) {
		ssi_wrn ("could not read stream from file '%s'", path);
		return false;
	}

	_sample_number_total = _file_stream_in.getTotalSampleSize ();
	if (_offset_in_samples > _sample_number_total) {
		ssi_wrn ("offset exceeds #samples in file (%u > %u)", _offset_in_samples, _sample_number_total);
		return false;
	}
...
bool FileReader::disconnect () {
	...
	ssi_msg (SSI_LOG_LEVEL_BASIC, "stop 'path=%s'", _options.path);
