		...
		if (_options.osc) {
			...
			switch (e->type) {
				...
				case SSI_ETYPE_NTUPLE: {			
					...
					result = _socket_osc->send_event (Factory::GetString (e->event_id),
						time,
						e->dur,
						e->state,
						n,						
				...
				case SSI_ETYPE_STRING: {
					...
					result = _socket_osc->send_message (Factory::GetString (e->event_id),
						time,
						e->dur,
						e->ptr);
			...
			if (_options.xml) {
				...
				switch (e->type) {
					...
					case SSI_ETYPE_FLOATS: {
						ssi_size_t num = e->tot / sizeof (ssi_real_t);						
						ssi_char_t string[SSI_MAX_CHAR];
						if (!ssi_array2string (num, ssi_pcast (ssi_real_t, e->ptr), SSI_MAX_CHAR, string)) {
							return false;
						}
					...
					case SSI_ETYPE_NTUPLE: {
						...
						ssi_event_tuple_t *ts = ssi_pcast (ssi_event_tuple_t, e->ptr);
