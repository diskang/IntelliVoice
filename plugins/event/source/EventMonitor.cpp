...
void EventMonitor::listen_enter () {
	...
	if (!_options.console && !_monitor) {
		...
		_window->setStyle(IWindow::STYLES::NO_CLOSE);
		_window->create();
		_window->show();		
	}
#endif
...
bool EventMonitor::update (IEvents &events, ssi_size_t n_new_events, ssi_size_t time_ms) {
	...
	if (_options.console) {
		...
		ssi_sprint (_string, "%s (time=%u)", _options.mname, time_ms);		
		...
		if (_options.all || n_new_events > 0) {
			...
			ssi_sprint(_string, "#   %-10.9s%-10.9s%-10.9s%-10.9s%-10.9s%-10.9s%-11.10s\r\n-------------------------------------------------------------------------------\r\n", "type", "sender", "event", "time", "dur", "size", "state(gid)");
			_monitor->print(_string);
			while (e = events.next()) {
				ssi_sprint(_string, "%03u %-10.9s%-10.9s%-10.9s%-9u %-9u %-9u %s(%02u)\r\n", count++, SSI_ETYPE_NAMES[e->type], Factory::GetString(e->sender_id), Factory::GetString(e->event_id), e->time, e->dur, e->tot, e->state == SSI_ESTATE_CONTINUED ? "continued" : "completed", e->glue_id);
				_monitor->print(_string);
				if (_options.detail) {
					switch (e->type) {
