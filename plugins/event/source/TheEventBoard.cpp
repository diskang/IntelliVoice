...
bool TheEventBoard::RegisterSender (IEventSender &sender) {
	...
	if (result) {
		ssi_msg(SSI_LOG_LEVEL_BASIC, "registered sender '%s'", sender.getEventAddress());
	}
	
	return result;	
}

bool TheEventBoard::RegisterListener (IEventListener &listener,		
	...
	for (ssi_size_t i = 0; i < n_sender; i++) {
		...
		if ((sender_ids[i] = Factory::GetStringId (s)) == SSI_FACTORY_STRINGS_INVALID_ID) {
			ssi_wrn ("could not register listener because sender '%s' was not found", s);			
			return false;
		}
	}
	...
	for (ssi_size_t i = 0; i < n_events; i++) {
		...
		if ((events_ids[i] = Factory::GetStringId (e)) == SSI_FACTORY_STRINGS_INVALID_ID) {
			ssi_wrn ("could not register listener because event '%s' was not found", e);			
			return false;
		}
	}
	...
	ssi_msg(SSI_LOG_LEVEL_BASIC, "registered listener '%s'", address);
