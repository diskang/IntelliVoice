...
void ConsumerBase::enter () {
	...
	ssi_msg (SSI_LOG_LEVEL_BASIC, "start '%s'", _consumer->getName ());
...
int ConsumerBase::consume (IConsumer::info info) {
	...
	SSI_DBG (SSI_LOG_LEVEL_DEBUG, "consumed %u samples ('%s')", _streams[0].num, _consumer->getName ());
...
void ConsumerBase::flush () {
	...
	ssi_msg (SSI_LOG_LEVEL_BASIC, "stop '%s'", _consumer->getName ());
...
bool ConsumerBase::check_trigger_stream (ssi_stream_t &s) {
	...
	switch (s.type) {
		...
		case SSI_USHORT: {			
			unsigned short *ptr = ssi_pcast (unsigned short, s.ptr);			
			for (ssi_size_t i = 0; i < n; i++) {
				if (*ptr++ != 0) {
					result = true;
					break;
		...
		case SSI_UINT: {			
			unsigned int *ptr = ssi_pcast (unsigned int, s.ptr);			
			for (ssi_size_t i = 0; i < n; i++) {
				if (*ptr++ != 0) {
					result = true;
					break;
		...
		case SSI_ULONG: {			
			unsigned long *ptr = ssi_pcast (unsigned long, s.ptr);			
			for (ssi_size_t i = 0; i < n; i++) {
				if (*ptr++ != 0) {
					result = true;
					break;
