...
Consumer::Consumer (int buffer_id,
	...
	// set thread name
	ssi_char_t *thread_name = ssi_strcat ("consume@", _consumer->getName ());
...
void Consumer::run () {
	...
	switch (status) {
		...
		case TimeBuffer::DATA_NOT_IN_BUFFER_YET:
			...
			SSI_DBG (SSI_LOG_LEVEL_DEBUG, "data not in buffer yet '%s'", _consumer->getName ());
			return;
		...
		default:
			...
			ssi_wrn ("requested data not available (%s) '%s'", TimeBuffer::STATUS_NAMES[status], _consumer->getName ());
