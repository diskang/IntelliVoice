...
void Selector::transform_enter (ssi_stream_t &stream_in,
	...
	else
	{
		...
		_offsetList = new int[_n_selected + 1];
		...
		_offsetList[_n_selected] = (stream_in.dim - _selectedIndexList[_n_selected - 1]) * sizeof(float);
	}
...
void Selector::transform (ITransformer::info info,
	...
	else
	{
		for (ssi_size_t i = 0; i < stream_out.num; i++) {
			...
			ptr_in += _offsetList[_n_selected];
		}
	}
}
...
void Selector::transform_flush (ssi_stream_t &stream_in,
	...
	delete[] _selectedIndexList; _selectedIndexList = 0;
	delete[] _offsetList; _offsetList = 0;
	_selectedIndexList = 0;
...
bool Selector::Parse (const ssi_char_t *indices) {
	...
	_selectedIndexList = ssi_parse_indices(indices, _n_selected, _options.sort, ",");
