...
#ifdef FRAMEWORK_LOG
{
	...
	#else
	uint64_t server_system_time;
	#endif
	ssi_size_t sync_delay = 0;
	
	if (_options.sync) {		
		if (_options.slisten) {
			_sync_socket = Socket::CreateAndConnect (_options.stype, Socket::SERVER, _options.sport, "");
			...
			_sync_socket = Socket::CreateAndConnect (_options.stype, Socket::CLIENT, _options.sport, "");	
		}
	}
	...
	if (_options.sync) {				
...
#else
			ssi_msg (SSI_LOG_LEVEL_BASIC, "received start message '%u with a delay of %ums", server_system_time ,sync_delay);
		...
		
		{
			...
			int result = sizeof(_system_time) ;
			result= _sync_socket->send (&_system_time, sizeof (_system_time));
			if (result <= 0) {
			...
			#else
			ssi_msg (SSI_LOG_LEVEL_BASIC, "sent start message  (port=%d protocol=%s)", _options.sport, _options.stype == Socket::UDP ? "udp" : "tcp");
...
#ifdef FRAMEWORK_LOG
{
	...
	if (_options.sync && _options.slisten) {
		...
		#else
		uint64_t server_system_time;
		uint64_t server_system_filetime;
		#endif
		int result = _sync_socket->recv (&server_system_time, sizeof (server_system_time));
		if (result <= 0) {
