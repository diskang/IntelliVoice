...
void Mouse::run () {
    ...
    _cursor_pos.x=x;
    _cursor_pos.y=y;
    short button_pressed=*buttons.mask && _options.mask;
	
    //printf("X, %f Y, %f B %d\n",_cursor_pos.x, _cursor_pos.y, button_pressed);
	#endif
...
void Mouse::sendEvent() {

	if (_elistener && _options.sendEvent) {
		_event.time = Factory::GetFramework()->GetRunTimeMs();
