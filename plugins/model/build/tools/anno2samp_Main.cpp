...
int main (int argc, char **argv) {
	...
	cmd.addSCmdOption("-label", &label, "garbage", "default label if not covered by an annotation segment (applied if frame > 0)");
	...
	if (cmd.read (argc, argv)) {		
		...
		if (frame > 0) {
			Annotation anno_c;
			ModelTools::ConvertToContinuousAnnotation(anno, anno_c, frame, delta, percent, label);
