...
void ex_fsel () {
	...
	// relief
	{
		Relief *relief = ssi_pcast (Relief, Factory::Create (Relief::GetCreateName (), "relief"));
		...
		FloatingSearch *fs = ssi_pcast (FloatingSearch, Factory::Create (FloatingSearch::GetCreateName (), "sfs"));
		fs->getOptions ()->nfirst = 0;
		SimpleKNN *model = ssi_pcast (SimpleKNN, Factory::Create (SimpleKNN::GetCreateName ()));
		fs->setModel (*model);
