...
void ex_pca (SampleList &samples) {
	...
	{ // build
		PCA *model = ssi_factory_create (PCA, "pca", false);	
	...
	{ // test
		...
		if (n_dimensions == 2) {
			...
			sreduced_norm.setNorm (ISNorm::INTERVAL);
...
void ex_fisher (SampleList &samples) {
	...
	{ // build
		Fisher *model = ssi_factory_create (Fisher, "LDA", false);	
	...
	{ // test
		...
		Fisher *model = ssi_factory_create (Fisher, "fisher", false);	
		model->load ("fisher");
		ssi_stream_t reduced;
