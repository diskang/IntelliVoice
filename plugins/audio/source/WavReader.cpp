...
void WavReader::setProvider (IProvider *provider) {
	...
	ssi_msg (SSI_LOG_LEVEL_BASIC, "open wav file '%s'", _options.path);

	// open file	
	_wav_file = File::CreateAndOpen (File::BINARY, File::READ, _options.path);	
	...
	_wav_file->seek (_offset_in_bytes, File::CURRENT);
...
WavReader::~WavReader() {
	...
	ssi_msg (SSI_LOG_LEVEL_BASIC, "close wav file '%s'", _options.path);
...
bool WavReader::connect () {
	...
	// set thread name
	ssi_char_t *thread_name = ssi_strcat ("ssi_sensor_WavReader@", _options.path);
...
void WavReader::run () {
	...
	if (_frame_counter > 0) {
		...
		if (_scale) {
			short *src = ssi_pcast (short, _stream.ptr);
		...
		if (_options.loop) {
			...
			ssi_msg (SSI_LOG_LEVEL_BASIC, "loop '%s'", _options.path);
