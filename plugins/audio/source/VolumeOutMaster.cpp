...
bool CVolumeOutMaster::Init()
{
	...
	{
		ssi_wrn (".MasterOutputVolume: FAILURE: Could not open WaveOut Mapper. mmResult=%d\n", mmResult );
		return false;
	} else {
		...
		if ( mmResult != MMSYSERR_NOERROR )
		{
			ssi_wrn (".MasterOutputVolume: FAILURE: WaveOut Mapper in Mixer is not available. mmResult=%d\n", mmResult );
			return false;
		}
	}
	...
	if ( !m_hWnd )
	{
		ssi_wrn (".MasterOutputVolume: FAILURE: Could not create internal window.\n" );
		return false;
	}
	...
	if ( mmResult != MMSYSERR_NOERROR )
	{
		ssi_wrn (".MasterOutputVolume: FAILURE: Could not open Mixer. mmResult=%d\n", mmResult );
...
bool CVolumeOutMaster::Initialize()
{
	...
	if ( !m_bOK )
		return false;
	ssi_wrn (".MasterOutputVolume: Initializing for Source MasterOut Line ..\n" );
	MIXERLINE MixerLine;
	...
	if ( mmResult != MMSYSERR_NOERROR )
	{
		ssi_wrn (".MasterOutputVolume: FAILURE: Could not get Speakers Destionation Line for the MasterOut Source Line while initilaizing. mmResult=%d\n", mmResult );
		return false;
	}
	...
	if ( mmResult == MMSYSERR_NOERROR )
	{
		...
		{
			m_bAvailable = true;
			ssi_wrn (".MasterOutputVolume: \"%s\" Volume control for the Speakers Source Line adopted.\n", Control.szShortName );
		} else {
			ssi_wrn (".MasterOutputVolume: WARNING: The Volume Control is disabled.\n" );
		}
	} else {
		ssi_wrn (".MasterOutputVolume: WARNING: Could not get the Speakers Source line Volume Control while initilaizing. mmResult=%d\n", mmResult );
	}
...
void CVolumeOutMaster::EnableLine( bool bEnable )
{
	...
	if ( mmResult != MMSYSERR_NOERROR )
	{
		if ( bEnable )
		{
			ssi_wrn (".MasterOutputVolume: FAILURE: Could not get the Speakers Destination Line while enabling. mmResult=%d\n", mmResult );
		} else {
			ssi_wrn (".MasterOutputVolume: FAILURE: Could not get the Speakers Destination Line while disabling. mmResult=%d\n", mmResult );
		}
		return;
	}
	...
	if ( !aControls )
	{
		if ( bEnable )
		{
			ssi_wrn (".MasterOutputVolume: FAILURE: Out of memory while enabling the line.\n" );
		} else {
			ssi_wrn (".MasterOutputVolume: FAILURE: Out of memory while disabling the line.\n" );
		}
		return;
	}
	...
	if ( mmResult == MMSYSERR_NOERROR )
	{
		for (int i = 0; i < nControlCount; i++ )
		{
			...
			if ( bReadyToSet )
			{
				...
				if ( mmResult == MMSYSERR_NOERROR )
				{
					if ( bEnable )
					{
						ssi_wrn (".MasterOutputVolume: Enabling Line: Speakers Line control \"%s\"(0x%X) has been set to %d.\n", aControls[i].szShortName, aControls[i].dwControlType, lValue );
					} else {
						ssi_wrn (".MasterOutputVolume: Disabling Line: Speakers Line control \"%s\"(0x%X) has been set to %d.\n", aControls[i].szShortName, aControls[i].dwControlType, lValue );
					}
					bAnyEnabled = true;
				}
				free( aDetails );
		...
		if ( bEnable )
		{
			ssi_wrn (".MasterOutputVolume: FAILURE: Could not get the line controls while enabling. mmResult=%d\n", mmResult );
		} else {
			ssi_wrn (".MasterOutputVolume: FAILURE: Could not get the line controls while disabling. mmResult=%d\n", mmResult );
		}
	}
	free( aControls );
	if ( !bAnyEnabled )
	{
		if ( bEnable )
		{
			ssi_wrn (".MasterOutputVolume: WARNING: No controls were found for enabling the line.\n" );
		} else {
			ssi_wrn (".MasterOutputVolume: WARNING: No controls were found for disabling the line.\n" );
		}
	}
}
//////////////////////////////////////////////
