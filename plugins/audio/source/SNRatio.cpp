...
void SNRatio::transform_enter (ssi_stream_t &stream_in,
	...
	_thres_s = ssi_cast (short, _options.thresh + 0.5);
...
void SNRatio::transform (ITransformer::info info,
	...
	if (input_is_float) {
		...
		short *inptr = ssi_pcast (short, stream_cast.ptr);
		for (ssi_size_t i = 0; i < stream_in.num * stream_in.dim; i++) {
			*inptr++ = ssi_cast (short, *fltptr++ * 32768.0f);
		}
		inptr = ssi_pcast (short, stream_cast.ptr);
		float value = ssi_cast (float, signal_to_noise_ratio_short (inptr, sample_number - 64));
		*outptr = value >= _thres_f ? value : 0;
	} else {
		short *inptr = ssi_pcast (short, stream_in.ptr);
		short *outptr = ssi_pcast (short, stream_out.ptr);
		short value = signal_to_noise_ratio_short (inptr, sample_number - 64);
...
int SNRatio::signal_to_noise_ratio_short (short *input, int N) {
...
int SNRatio::covariance_short (short *x, short *y, int N) {
...
int SNRatio::variance_short (short *x, int N) {
