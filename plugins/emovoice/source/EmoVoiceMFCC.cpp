...
ssi_size_t EmoVoiceMFCC::getSampleBytesOut (ssi_size_t sample_bytes_in) {
	...
	SSI_ASSERT (sample_bytes_in == sizeof (short));
...
void EmoVoiceMFCC::transform (ITransformer::info info,
	...
	short *inblock = ssi_pcast (short, stream_in.ptr);
	...
	for (ssi_size_t i = 0; i < steps; i++) {
		dsp_fextract_calc (fex, (mx_real_t *) outblock, (dsp_sample_t *) (inblock + i*EMOVOICEMFCC_FRAME_STEP));
