...
void SocketSpy::message (const char *from,
	...
	ssi_msg (SSI_LOG_LEVEL_BASIC, "%s -> /text %s %dms %dms %s", from, id, time, dur, msg);
};
...
void SocketSpy::event (const char *from,
	...
	ssi_sprint (_string, "%s -> /evnt %s %dms %.2dms (%s) n=%d", from, id, time, dur, state == SSI_ESTATE_COMPLETED ? "completed" : "continued", event_size);
	ssi_print ("%s\n", _string);
