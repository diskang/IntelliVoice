...
void ModelTools::LoadAnnotation (Annotation &annotation, File &file) {
	...
	while (file.ready ()) {
		...
		// parse line
		num = sscanf (line, "%lf %lf %s ", &start_time, &stop_time, label_name);
		if (num == 3) {
			SSI_DBG (SSI_LOG_LEVEL_DEBUG, "found new entry: %f %f %s\n", start_time, stop_time, label_name);
		} else {
			ssi_wrn ("ignored line");
			continue;
		}
	...
	while (entry = annotation.next ()) {
		...
		for (ssi_size_t j = 0; j < num; j++) {
			...
			if (! (start_index <= stop_index && stop_index < streams[j]->num)) {
				ssi_wrn ("invalid interval [%lf..%lf]s", entry->start, entry->stop);
				continue;
			}
