...
bool SampleList::addSample (ssi_sample_t *sample, bool deep_copy) {
	...
	if (sample->user_id >= _users.size() && sample->class_id != SSI_ISAMPLES_GARBAGE_USER_ID) {
		ssi_wrn ("sample exceeds user id");
		return false;
	}

	if (sample->class_id >= _classes.size (	) && sample->class_id != SSI_ISAMPLES_GARBAGE_CLASS_ID) {
