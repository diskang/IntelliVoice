...
: Buffer (ssi_cast (int, capacity * sample_rate_) * sample_bytes_ * sample_dimension_), 
	  max_samples (ssi_cast (int, capacity * sample_rate_)),
...
TimeBuffer::STATUS TimeBuffer::get (ssi_byte_t **data, ssi_size_t &samples_in, ssi_size_t &samples, ssi_time_t start_time, ssi_time_t duration) {
	...
	ssi_size_t start_sample = ssi_cast (ssi_size_t, (start_time * sample_rate) + 0.5);
	ssi_size_t stop_sample = ssi_cast (ssi_size_t, ((start_time + duration) * sample_rate) + 0.5);
...
TimeBuffer::STATUS TimeBuffer::get (ssi_byte_t *data, ssi_size_t samples, ssi_size_t start_sample) {
...
void TimeBuffer::reset (ssi_time_t offset) {
	...
	offset_samples = ssi_cast (int, offset * sample_rate);
	next_sample = 0;
	last_accessed_sample = 0;
	buffer_filled = false;
}
...
ssi_size_t TimeBuffer::getCurrentWritePos () {
...
void TimeBuffer::setCurrentSampleTime (ssi_time_t sync_time) {
	...
	offset_samples -= ssi_cast (int, delta * sample_rate + 0.5);
