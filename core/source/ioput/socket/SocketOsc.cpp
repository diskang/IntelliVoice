...
SocketOsc::SocketOsc (Socket &socket,
	...
	_recv_buffer = new char[_buffer_size];	
...
int SocketOsc::send_message (const ssi_char_t *id, 
	osc_int32 time,
	osc_int32 dur,
...
int SocketOsc::send_event (const ssi_char_t *id,
	osc_int32 time,
	osc_int32 dur,
	osc_int32 state,
	osc_int32 n_events,
	...
	if (result > 0) {
		...
		try {
			...
			if (strcmp (pattern, SocketOsc::STREAM_PATTERN) == 0) {
				...
				if (parse_event (args, id, time, dur, state, n_events, _event_name_buffer, _event_values_buffer)) {
					listener->event (_socket.getRecvAddress (), id, time, dur, state, n_events, _event_name_buffer, _event_values_buffer);
				} else {
				...
				osc_int32 time, dur;
				const ssi_char_t *string;
				if (parse_message (args, id, time, dur, string)) {
					listener->message (_socket.getRecvAddress (), id, time, dur, string);
				} else {
...
bool SocketOsc::parse_event (ReceivedMessageArgumentStream &args,
	...
	args >> id >> time >> dur >> state >> n_events;	
...
bool SocketOsc::parse_message (ReceivedMessageArgumentStream &args,
	...
	args >> id >> time >> dur >> string >> EndMessage;
