...
<HTML>\n\
	<HEAD>\n\
		<TITLE>Social Signal Interpretation (SSI) API [v0.9.5]\n\
		</TITLE>\n\
	</HEAD>\n\
		...
		<TABLE WIDTH=\"100%\" HEIGHT=\"100%\">\n\
			<TR>\n\
				<TD ALIGN=\"CENTER\" VALIGN=\"MIDDLE\">\n\
					...
					<P>Built with Social Signal Interpretation (SSI) [v0.9.5]\n\
						<BR />(c) 2007-14 University of Augsburg\n\
						<BR />\n\
...
bool APIGenerator::CreateMenu (const ssi_char_t *apidir) {
	...
	for (ssi_size_t i = 0; i < files.size (); i++) {
...

		{
			...
			table_tr_td_code_a.InsertEndChild (TiXmlText (fp.getName ()));
...
bool APIGenerator::CreateAPI (const ssi_char_t *filepath) {
	...
	cp_p.InsertEndChild (TiXmlText ("(c) 2007-14 University of Augsburg"));
	cp_p.InsertEndChild (cp_p_br);
...
bool APIGenerator::CreateClass (TiXmlElement &body, const ssi_char_t *create) {
	...
	if (strcmp (oname, ThePainter::GetCreateName ()) != 0 && 
		strcmp (oname, TheEventBoard::GetCreateName ()) != 0 &&
