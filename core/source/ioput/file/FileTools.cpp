...
bool FileTools::ReadSampleHeader (File &file,
	...
	switch (file.getType ()) {
		...
		case File::ASCII: {
			...
			ssi_char_t string[1024];
			if (!file.readLine (1024, string)) {
				ssi_err ("could not read <user_id> <class_id> <time> <prob> <num>");
			}
			sscanf (string, "%u %u %lf %f %u", &data.user_id, &data.class_id, &data.time, &data.prob, &data.num);
		...
		case File::BINARY: {
			...
			if (!file.read (&data.prob, sizeof (data.prob), 1)) {
				ssi_err ("could not read <prob>");
			}
...
void FileTools::WriteSampleHeader (File &file,
	...
	switch (file.getType ()) {
		...
		case File::ASCII: {
			...
			ssi_char_t string[1024];
			sprintf (string, "%u %u %lf %.2f %u", data.user_id, data.class_id, data.time, data.prob, data.num);
			file.writeLine (string);
		...
		case File::BINARY: {
			...
			file.write (&data.time, sizeof (data.time), 1);
			file.write (&data.prob, sizeof (data.prob), 1);
			file.write (&data.num, sizeof (data.num), 1);
