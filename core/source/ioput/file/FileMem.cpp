...
bool FileMem::move (ssi_size_t newpos) {
	_pos = newpos;
	if (_pos > _n_memory - (_type == FileMem::ASCII ? 1 : 0)) {
		ssi_err ("position out of memory (%u > %u)", _pos, _n_memory - (_type == FileMem::ASCII ? 1 : 0));		
	}		
	return true;
}

bool FileMem::seek (long int offset, FileMem::ORIGIN origin) {
