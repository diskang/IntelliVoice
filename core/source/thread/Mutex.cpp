...
Mutex::~Mutex () {
 ...
 std::unique_lock<std::mutex> Mutex::unique_lock()
{

    std::unique_lock<std::mutex>lock(critSec, std::defer_lock);
    lock.try_lock(); //(std::chrono::milliseconds (1));
    return lock;
}
//special construct to keep interface and behavior of windows implementation
 std::unique_lock<std::mutex> Mutex::cond_var_unique_lock()
{
    release();
    std::unique_lock<std::mutex>lock(critSec, std::defer_lock);
    lock.try_lock(); //(std::chrono::milliseconds (1));
