...
#if HAS_CAIRO
void GraphicTools::paintStreamAsImage2Buffer(ssi_stream_t &stream, ssi_rect_t imgRes, cairo_surface_t** surf, IColormap &colormap)
{
	
	ssi_int_t vals_x = imgRes.width;
	ssi_int_t vals_y = imgRes.height;
	ssi_real_t *ptr = ssi_pcast(ssi_real_t, stream.ptr);
	*surf= cairo_image_surface_create (
	...
	for(int i=0; i < vals_x*vals_y; i++)
	{
		...
		uint32_t color=colormap.getColor(*(ptr+i));
...
void GraphicTools::PaintAsImage(ssi_stream_t &stream, IPainter &painter, IColormap &colormap) {
	...
	if (stream.type != SSI_REAL) {
		...
		paintStreamAsImage2Buffer(stream,{0,0,stream.num, stream.dim}, &img, colormap);
