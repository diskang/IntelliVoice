...
public:
	...
	virtual bool read (void *ptr, ssi_size_t size, ssi_size_t count) = 0;
	virtual bool write (const void *ptr, ssi_size_t size, ssi_size_t count) = 0;
	virtual bool readLine (ssi_size_t num, ssi_char_t *string) = 0;
	virtual bool writeLine (const ssi_char_t *string) = 0;

	virtual void setFormat (const char *delim, const char *flags) {};
	virtual void setType (ssi_type_t type) {};

	bool seek (long int offset, FileMem::ORIGIN origin = FileMem::BEGIN);
	ssi_size_t tell ();
	bool ready ();
