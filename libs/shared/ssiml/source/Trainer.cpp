	...
	if (element = body->FirstChildElement ("model")) {
		...
		if (create_model) {
			...
			if (optpath) {
				...
				trainer._models[0] = ssi_pcast(IModel, Factory::Create(create, string));
			} else {
				trainer._models[0] = ssi_pcast(IModel, Factory::Create(create));
			}
		}
		...
		if (!trainer._fusion) {
			...
			if (optpath) {
				ssi_sprint (string, "%s%s", fp.getDir (), optpath);
				trainer._fusion = ssi_pcast(IFusion, Factory::Create(create, string));
			} else {
				trainer._fusion = ssi_pcast(IFusion, Factory::Create(create));
			}
		}
		...
		for (ssi_size_t n_model = 0; n_model < n_models; n_model++) {
			...
			if (create_models) {
				...
				if (optpath) {
					ssi_sprint (string, "%s%s", fp.getDir (), optpath);
					trainer._models[n_model] = ssi_pcast(IModel, Factory::Create(create, string));
				} else {
					trainer._models[n_model] = ssi_pcast(IModel, Factory::Create(create));
				}
			}
...
bool Trainer::save_V5 (const ssi_char_t *filepath, TiXmlElement &body) {
	...
	if (_has_transformer) {
		...
		for (ssi_size_t n = 0; n < _n_streams; n++) {
			if (_transformer[n]) {
				...
				item.SetAttribute ("help", _transformer[n]->getInfo ());	
				transform.InsertEndChild (item);
			}
		}
		body.InsertEndChild (transform);
	...
	if (_fusion) {
		...
		fusion.SetAttribute ("help", _fusion->getInfo ());
		...
		for (ssi_size_t n_model = 0; n_model < _n_models; n_model++) {
			...
			item.SetAttribute ("help", _models[n_model]->getInfo ());
			models.InsertEndChild (item);
		}
		fusion.InsertEndChild (models);
		...
		if (_models[0]->getOptions ()) {
			...
			FilePath fp (filepath);
			ssi_sprint (string, "%s%s.%s", fp.getName (), SSI_FILE_TYPE_TRAINER, _models[0]->getName ());
			model.SetAttribute ("option", string);					
		}
		model.SetAttribute ("help", _models[0]->getInfo ());
		body.InsertEndChild (model);
	}
