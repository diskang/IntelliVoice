...
MLP *create (const ssi_char_t *dir) {
	...
	MLP *pipe = ssi_pcast (MLP, Factory::Create (MLP::GetCreateName (), "pipe"));
	pipe->setDir (dir);

	Mouse *mouse = ssi_pcast (Mouse, Factory::Create (Mouse::GetCreateName (), "mouse"));
	mouse->getOptions ()->mask = Mouse::LEFT;

	Dollar$1 *dollar = ssi_pcast (Dollar$1, Factory::Create (Dollar$1::GetCreateName (), "dollar"));
	Selector *select = ssi_pcast (Selector, Factory::Create (Selector::GetCreateName (), "select"));
